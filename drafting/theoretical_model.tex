...
\title{MPhil Thesis: Theoretical Model}
...
\subsection{Stage 1: The Innovation Decision (Effort)}

First, firms make investment decisions for a drug they wish to market within therapeutic class $c$. Innovation is modelled to require effort, which raises the probability of clinical success while being costly, consistent with \citeauthor{sutton1998technology} characterising pharmaceutical R\&D as a search process where greater research effort raises the likelihood of success.

\subsubsection{The Production Technology}
...
\subsubsection{The Cost of Effort}
...
\subsection{Stage 2: The P\&T Stage}
...
\subsubsection{The Approval Condition}
...
\subsubsection{The Probability of Success \texorpdfstring{($Q_c$)}{(Qc)}}
...
\begin{align}
    Q_c(e_{i,c}) &= \Pr \left(\ln(e_{i,c}) + \varepsilon_i \geq \bar{a} \right) \\
           &= \Pr \left( \varepsilon_i \geq \bar{a} - \ln(e_{i,c}) \right) \\
           &= 1 - \Pr  \left(\varepsilon_i < \bar{a} - \ln(e_{i,c}) \right) \\
...
\subsection{Stage 3: The Pricing Subgame}
...
\subsubsection{Stage 3a: The Monopoly State}
...
\subsubsection{Stage 3b: The Duopoly State (Auction)}
...
\subsubsection{Stage 3c: The Failure State}
...
\subsubsection{Optimal Effort \texorpdfstring{$(e_c^*)$}{(ec*)}}
...
\subsubsection{The ``Incentive Trap'' Mechanism}
...
Based on \ref{eq:tau_steepness} $\tau_c'(M_c)$ is increasing in $n_c$, so classes with more substitutes have a lower threshold $M_c^*$. Therefore, classes with more interchangeable drugs are likelier to meet the threshold market size and become trap-bound.
\end{result}
...
\subsubsection{Institutional and Empirical Justification}
...
Table~\ref{tab:rebate_regression} shows that the coefficients on both $\log(M_c)$ and $n_c$ are positive and significant, indicating that larger Medicaid markets and a greater number of distinct drugs within a class are both associated with higher rebates. These results support the assumptions that $\tau_c'(M_c)>0$ and that the rebate schedule steepens with $n_c$.

%TC:ignore
\input{../output/rebate_exploration/rebate_regression.tex}
%TC:endignore

\subsubsection{State-Dependent Innovation}
...
In this setting, period 1 procurement outcomes in class $c$ determine the firm's position in Period 2. A firm may fail to gain access, gain access under competitive rents, or gain access under higher monopoly rents. A natural explanation for this state dependence is the internal-finance channel. \cite{hal2002financing} shows that R\&D is especially exposed to financing frictions because it is risky and intangible, making external finance costly. This concern is amplified in pharmaceuticals, where drug development involves substantial sunk costs, highly variable returns, and informational asymmetries that disadvantage firms with limited internal resources \citep{sutton1998technology}. Securing preferred access on the PDL generates additional cash flows that relax financing constraints, thereby lowering the effective marginal cost of future R\&D.

Additionally, firms that pass the P\&T stage and negotiate rebates may gain organisational knowledge specific to navigating Medicaid procurement. This lowers the effective cost of future innovation without relying on pharmaceutical discovery itself accumulating across research trajectories. This modelling choice is supported by \citeauthor{sutton1998technology} (\citeyear{sutton1998technology}), who documents limited persistence in firms' follow-on drug successes. This makes cash flow and procurement-related capabilities a more appropriate basis for state-dependent innovation than persistent dominance in drug discovery.
...
\subsection{Model Predictions and Empirical Mapping}

\subsubsection{Comparative Statics} \label{sec:comp_statics}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{../output/theoretical_model/comparative_statics.png}
    \caption{Comparative statics of equilibrium effort $e_c^*$. Baseline parameter values are reported in Table~\ref{tab:parameters}.}
...
\subsubsection{Theoretical States and Empirical Outcomes}
...
\paragraph{Mapping to the PDL Counterfactuals}
...
\paragraph{Mapping to the Triple-Difference} \label{sec:rq3_mapping}
...
\toprule
Category & Sign of $\beta_c$ & Condition & Prediction \\
\midrule
...
\section{Baseline Model Parameters} \label{app:parameters}
