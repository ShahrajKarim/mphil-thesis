...
\begin{enumerate}[label=(\roman*)]
    ...
    \item A rebate function $\tau_c(M_c)$ with $\tau_c' > 0$. This determines how aggressive supplemental rebates from Medicaid are at a per-prescription level.
...
The model does not impose a functional form on $\tau_c$ - the only requirement is that $\tau_c' > 0$. Additionally, the steepness of the rebate schedule is linked to the degree of therapeutic substitutability, that is:
...
Since the denomininator is the SOC (which is known to be strictly negative), the sign of the best response derivative depends purely on the sign of the cross-partial derivative:
...
Based on Equation \ref{eq:tau_steepness} $\tau_c'(M_c)$ is increasing in $n_c$, so classes with more substitutes have a lower threshold $M_c^*$. Therefore, classes with more interchangeable drugs are likelier to meet the threshold market size and become trap-bound.
\end{result}
...
To characterise the predictions of the model, equilibrium effort is solved for numerically from the FOCs. Since no closed-form solution exists for $e_c^*$, the comparative statics are computed by varying one parameter and holding all else fixed at baseline values reported in Table \ref{tab:parameters}. Since the functional form of $\tau_c(M_c)$ is unknown, three functional forms are proposed for the simulations: a power specification $\tau_c = \alpha_c M_c^{\beta_c}$, a logarithmic specification $\tau_c = \alpha_c \ln M_c$, and a linear specification $\tau_c = \alpha_c M_c$. The resulting comparative statics are shown in Figure \ref{fig:comp_statics}.
