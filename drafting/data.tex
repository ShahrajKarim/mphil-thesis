...
The final constructed dataset is a balanced panel at the firm $\times$ ATC Level 1 $\times$ year $\times$ quarter level, spanning from 2010 to 2025. The dataset uses observations from 2010 since the constructed pre-expansion Medicaid exposure measures utilise at least two years of pre-reform prescription data - the SDUD is downloaded from 2005 onwards for this reason. All datasets are harmonised to the World Health Organisation's Anatomical Therapeutic Chemical (ATC) classification system at Level 1, as described in Section~\ref{sec:atc}. Table~\ref{tab:source_descriptives} reports descriptive statistics for each of the four datasets used in the analysis.
...
\begin{tabular}{lllll}
\toprule
\multicolumn{1}{c}{State 1} & \multicolumn{1}{c}{State 2} & \multicolumn{1}{c}{Overlap (State 1 in State 2)} & \multicolumn{1}{c}{Overlap (State 2 in State 1)} \\
\midrule
...
The A61P subclass is divided into CPC groups corresponding to different therapeutic uses. A manually constructed crosswalk is used map each A61P group to an ATC Level 1 class, with the crosswalk following the structure of the USPTO's A61P classification. Selected entries from this crosswalk are shown in Table~\ref{tab:a61p_atc_crosswalk}. For example, patents related to cardiovascular activity are classified under A61P9, so these are mapped to ATC class C, while patents related to nervous system activity fall under A61P25, thus they are mapped to ATC class N. In cases where a patent falls under multiple different A61P CPC groups, it is counted once within each applicable class.
...
\bottomrule
\multicolumn{3}{p{0.7\textwidth}}{\footnotesize\textit{Note:} $N$ denotes the number of unique firm names} \\
\end{tabular}
\end{table}
...
The final panels are constructed separately for the patent and FDA approval outcomes. Both patent counts and FDA approvals are collapsed to the firm $\times$ ATC Level 1 $\times$ year $\times$ quarter level. The SDUD prescription and reimbursement variables are first aggregated to the firm $\times$ ATC Level 1 $\times$ year $\times$ quarter level and are then merged with the innovation panels. PDL variables from the Texas Vendor Drug Program are merged at the firm $\times$ ATC Level 1 level - details relating to their exact construction are described in the Methodology section.

Each panel is balanced over the firm-class combinations observed in the data. In cases where no patent/approval observations for a given firm-class-year-quarter, an outcome value of zero is assigned. Class-level demand variables are calculated at the class-year-quarter observations, so that firms active in the same therapeutic class and quarter share the same class-level prescription aggregates. Summary statistics for each of the balanced panels are reported in Table~\ref{tab:panel_descriptives}. Histograms for variation in constructed PDL exposure variable in the interval $(0,1]$ can be found in Figure~\ref{fig:pdl_histogram}.
